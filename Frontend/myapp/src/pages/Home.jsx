...
const TextBox = ({ message, setMessage, sendMessage }) => (
    <div className="p-4 w-full">
        <div className="relative flex items-center bg-zinc-800 border border-gray-700 rounded-xl shadow-lg backdrop-blur-sm p-2 max-w-2xl mx-auto w-full">
            <input
                type="text"
...
export default function Home() {
    ...
    const handleNewChat = useCallback(async () => {
        ...
        try {
            ...
            const formattedNewChat = { ...newChat, id: newChat.id || newChat._id, messages: [] };
    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            ...
            try {
                ...
                console.log("📚 Received chat history with messages:", chatsFromServer);
    ...
    useEffect(() => {
        ...
        const handleAiMessage = (data) => {
            ...
            setChatHistory((prevChatHistory) => {
                const chatIndex = prevChatHistory.findIndex((chat) => String(chat.id) === String(chatId));
                if (chatIndex === -1) return prevChatHistory;
                const newChatHistory = [...prevChatHistory];
                const oldChat = newChatHistory[chatIndex];
                const existingMessages = oldChat.messages || [];
                const newMessage = { id: uuidv4(), role: "model", content: data.content };
                const updatedChat = { ...oldChat, messages: [...existingMessages, newMessage] };
    ...
    const sendMessage = async () => {
        ...
        let chatIdForSocket = activeChatId;
        let historyForSocket = [];
        if (chatIdForSocket === null) {
            try {
                ...
                setActiveChatId(newChatForState.id);
                chatIdForSocket = newChatForState.id;
                historyForSocket = [userMessage];
            } catch (error) {
                console.error("❌ Error creating new chat on the fly:", error);
                return;
            }
        } else {
            historyForSocket = chatHistory.find((chat) => String(chat.id) === String(chatIdForSocket))?.messages || [];
            setChatHistory(prev =>
                prev.map(chat =>
                    String(chat.id) === String(chatIdForSocket)
                    ...
                    {isLoadingHistory ? (
                        ...
                        chatHistory.map((chat) => (
                            <div
                                ...
                                className={`flex items-center gap-3 p-3 m-2 rounded-md cursor-pointer text-sm ${
                ...
                {activeChat && (
                    <div className="absolute top-4 right-4 z-20">
                        <button
                            ...
                            disabled={isSummarizing || !activeChat || activeChat.messages.length < 2}
                            className="flex items-center gap-2 p-2 bg-gray-800/50 rounded-md hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed"
                        ...
                        <div className="flex-1 overflow-y-auto p-6 space-y-4">
                            {activeChat.messages && activeChat.messages.map((msg) => (
                                ...
                                >
                                    ...
                                    <div
                                        className={`max-w-xl px-4 py-2 rounded-2xl whitespace-pre-wrap text-sm leading-relaxed ${
